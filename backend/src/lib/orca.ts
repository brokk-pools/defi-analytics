...
// Funções auxiliares para cálculo de fees
function toBN(value: any): bigint {
 ...
 * @param queryParams - String completa de query parameters (ex: "sortBy=volume&sortDirection=desc&stats=5m")
 */
export async function fetchPoolsFromOrcaAPI(queryParams?: string): Promise<any> {
  try {
    let url = 'https://api.orca.so/v2/solana/pools';
    
    // Adicionar query parameters se fornecidos
    if (queryParams && queryParams.trim()) {
      // Garantir que a string comece com '?' se não começar
      const cleanQueryParams = queryParams.trim().startsWith('?') ? queryParams.trim().slice(1) : queryParams.trim();
      url += `?${cleanQueryParams}`;
    }
    ...
    if (data.lasterror) {
      ...
      // Se há query parameters, tentar novamente sem eles
      if (queryParams && queryParams.trim()) {
        console.log('🔄 Tentando novamente sem parâmetros de query...');
 ...
 */
// Nova função que aceita apenas positionId e calcula fees diretamente
// Nova função que aceita apenas positionId para fees coletadas
export async function getCollectedFeesForPositionById(
  positionPkStr: string,
  startUtcIso?: string,
  endUtcIso?: string
): Promise<any> {
  try {
    console.log(`💰 Calculando fees coletadas para posição: ${positionPkStr}`);
    
    // Usar GetInnerTransactionsFromPosition diretamente
    ...
    console.log(`✅ Fees coletadas calculadas com sucesso para posição: ${positionPkStr}`);
    return transactionsResult;

  } catch (error) {
    console.error('❌ Error calculating collected fees by position ID:', error);
    throw error;
  }
}

export async function getOutstandingFeesForPositionById(
  positionPkStr: string
): Promise<any> {
  try {
    console.log(`💰 Calculando fees pendentes para posição: ${positionPkStr}`);
    ...
    try {
      ...
      console.log(`🔍 Tentando usar como NFT mint: ${positionPkStr}`);
    } catch (error) {
      throw new Error(`Invalid position ID format: ${positionPkStr}`);
    }

    // Se é um NFT mint, precisamos descobrir o endereço da posição
    const positionPda = PDAUtil.getPosition(ORCA_WHIRLPOOL_PROGRAM_ID, posPk);
    const positionPdaPk = positionPda.publicKey;
    
    console.log(`🎯 PDA da posição: ${positionPdaPk.toBase58()}`);

    // Buscar a posição usando o SDK
    const position = await client.getPosition(positionPdaPk);
    const positionData = position.getData();
    
    // Buscar a pool para obter os mints dos tokens
    const pool = await client.getPool(positionData.whirlpool);
    const poolData = pool.getData();
    const poolPk = positionData.whirlpool;
    ...
    // Calcular preços atuais para USD
    const [priceA, priceB] = await Promise.all([
      getPriceUSD(mintA.toBase58()),
    ...
    const result = {
      timestamp: new Date().toISOString(),
      method: 'getOutstandingFeesForPositionById',
      positionId: positionPkStr,
      positionAddress: positionPdaPk.toBase58(),
    ...
    console.log(`✅ Fees pendentes calculadas com sucesso para posição: ${positionPkStr}`);
    return result;

  } catch (error) {
    console.error('❌ Error calculating outstanding fees by position ID:', error);
    throw error;
  }
}
...
export async function feesCollectedInRange(
  ...
  try {
    ...
    if (positionId) {
      ...
      // Retornar exatamente o mesmo formato que GetInnerTransactionsFromPosition
